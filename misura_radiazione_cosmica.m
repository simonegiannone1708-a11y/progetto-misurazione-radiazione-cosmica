...
% ========== CONFIGURAZIONE INIZIALE (Modalità Dati + Categoria) ==========
fprintf('\n');
fprintf('╔════════════════════════════════════════════════════════════╗\n');
fprintf('║     CONFIGURAZIONE CALCOLO RADIAZIONE COSMICA              ║\n');
fprintf('╚════════════════════════════════════════════════════════════╝\n');
fprintf('\n');

% Opzioni modalità dati
modalita_dati = { ...
...
% Creazione finestra unica con tendine
fig_config = figure('Name', 'Configurazione Volo', ...
    'NumberTitle', 'off', ...
    'Position', [500, 300, 450, 250], ...
    'MenuBar', 'none', ...
    'Resize', 'off');

% Label e tendina modalità dati
uicontrol('Style', 'text', ...
    'String', 'Modalità dati space weather:', ...
    'Position', [20, 180, 200, 25], ...
    'HorizontalAlignment', 'left', ...
    'FontSize', 10, ...
    'FontWeight', 'bold');

popup_modalita = uicontrol('Style', 'popupmenu', ...
    'String', modalita_dati, ...
    'Position', [20, 150, 410, 25], ...
    'FontSize', 9);

% Label e tendina categoria utente
uicontrol('Style', 'text', ...
    'String', 'Categoria utente:', ...
    'Position', [20, 110, 200, 25], ...
    'HorizontalAlignment', 'left', ...
    'FontSize', 10, ...
    'FontWeight', 'bold');

popup_categoria = uicontrol('Style', 'popupmenu', ...
    'String', categorie_utenti, ...
    'Position', [20, 80, 410, 25], ...
    'FontSize', 9);

% Pulsante conferma
btn_conferma = uicontrol('Style', 'pushbutton', ...
    'String', 'Conferma', ...
    'Position', [160, 25, 130, 35], ...
    'FontSize', 11, ...
    'FontWeight', 'bold', ...
    'Callback', @(src, evt) uiresume(fig_config));

% Attendi selezione utente
uiwait(fig_config);

% Verifica se finestra chiusa senza conferma
if ~ishandle(fig_config)
    fprintf('Selezione annullata.\n');
    return;
end

% Recupera valori selezionati
id_modalita = get(popup_modalita, 'Value');
id_categoria = get(popup_categoria, 'Value');
close(fig_config);
...
if usa_dati_storici
    ...
    fprintf('Evento SEP storico selezionato:\n');
    ...
    fprintf('Utilizzando dati in tempo reale da NOAA SWPC...\n\n');
end
